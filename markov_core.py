...
class MarkovTextGenerator:
    ...
    def generate(self, start_state: tuple[str, ...] | list[str], max_tokens: int = 30) -> str:
        if not self.transitions:
            raise MarkovError("Модель не обучена. Сначала вызовите train_from_file(...).")
...
if __name__ == "__main__":
    ...
    training_file = "sample_training_text.txt"
    generator.train_from_file(training_file)
    print("Количество состояний:", len(generator.transitions))
    print(generator.generate(("это", "пример"), max_tokens=20))
